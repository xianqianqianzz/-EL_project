(async function initAdminTrips() {
  ...
  function render() {
    ...
    const filtered = trips.filter(trip =>
      ...
      (!keyword || `${trip.username} ${trip.display_name} ${trip.title} ${trip.from_label} ${trip.to_label}`.toLowerCase().includes(keyword))
    );
    target.innerHTML = filtered.length ? `<table class="admin-table"><thead><tr><th>用户</th><th>日程</th><th>路线</th><th>规则</th><th>提醒</th></tr></thead><tbody>${filtered.map(trip => `<tr>
