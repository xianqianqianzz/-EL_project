...
后台用户页在前端按角色分类，并使用用户 ID 将上述两类数据组合为用户详情。历史显示名无法正常读取时，接口回退为用户名，避免直接展示乱码。

日程与路径申请直接使用数据库主键作为稳定编号，界面显示为 `日程 #ID` 与 `申请 #ID`。后台日程和路径申请页面支持按编号查询，用户详情同时展示该用户关联记录的编号和摘要。
