...
1. 检查系统是否安装 Python 3。
2. 首次运行时自动安装缺失的后端依赖并初始化数据库。
3. 启动同源后端与网页服务，并打开登录页面 `http://localhost:8000/`。

若 `.exe` 无法使用，也可以双击 `一键打开校园地图.cmd`。

一键启动不再要求 `8080` 端口，因为 FastAPI 已经在 `8000` 端口同时提供网页和 API。开发时仍可使用前后端分离脚本启动 `8080` 前端。

将项目复制到其他 Windows 电脑时，目标电脑仍需预先安装 Python 3.10 或更高版本。首次启动需要联网下载 Python 依赖，耗时可能较长。启动失败时程序会显示具体原因，详细日志位于 `%LOCALAPPDATA%\nju-campus-map\launcher.log`。
